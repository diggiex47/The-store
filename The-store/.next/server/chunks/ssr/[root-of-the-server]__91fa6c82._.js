...
{
__turbopack_context__.s({
    "default": (()=>__TURBOPACK__imported__module__$5b$externals$5d2f$crypto__$5b$external$5d$__$28$crypto$2c$__cjs$29$__["randomFillSync"])
});
...
const authOptions = {
    ...
    callbacks: {
        async jwt ({ token, user }) {
            ...
            // generate apiToken if not exists
            if (!token.apiToken && token.id) {
                const secret = process.env.NEXTAUTH_SECRET;
                if (!secret) throw new Error("Missing NEXTAUTH_SECRET");
                const encryptionKey = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$panva$2f$hkdf$2f$dist$2f$node$2f$esm$2f$index$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["hkdf"])("sha256", Buffer.from(secret, "base64"), "next-auth-encryption", "NextAuth.js Generated Encryption Key", 32);
                const apiToken = await new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$jose$2f$dist$2f$node$2f$esm$2f$jwt$2f$encrypt$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["EncryptJWT"]({
                    sub: String(token.id),
                    email: token.email,
                    name: token.name
                }).setProtectedHeader({
                    alg: "dir",
                    enc: "A256GCM"
                }).setIssuedAt().setExpirationTime("8h").encrypt(encryptionKey);
                token.apiToken = apiToken;
            }
            // handle rememberMe
            const remember = token.rememberMe === true;
            token.exp = Math.floor(Date.now() / 1000) + (remember ? 60 * 60 * 24 * 8 : 60 * 60 * 24);
            return token;
        },
        async session ({ session, token }) {
            if (token) {
                ...
                session.apiToken = token.apiToken; // expose apiToken to frontend
            }
            return session;
        }
    }
...
//# sourceMappingURL=%5Broot-of-the-server%5D__91fa6c82._.js.map